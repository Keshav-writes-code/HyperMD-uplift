...
export const defaultClickHandler: ClickHandler = (info, cm) => {
  ...
  if (type === 'todo') {
    const { from, to } = expandRange(cm, pos, 'formatting-task')
...
const makeBackButton = (function () {
  let bookmark: { find(): CodeMirror.Position; clear() } = null
  ...
  function makeButton(cm: cm_t) {
    const hasBackButton = cm.options.gutters.indexOf('HyperMD-goback') != -1
    ...
    let _tmp1 = cm.display.gutters.children
  ...
  return function (cm: cm_t, line: number, anchor: CodeMirror.Position) {
    const backButton = makeButton(cm)
...
export class Click implements Addon.Addon, Options {
  ...
  constructor(public cm: cm_t) {
    ...
    const el = (this.el = cm.getWrapperElement())
  ...
  private _mouseDown = (ev: MouseEvent) => {
    ...
    if ((mat = styles.match(/\s(image|link|url)\s/))) {
      ...
      if (/^(?:image|link)$/.test(type) && !isBareLink) {
        ...
        const tmp_range = expandRange(cm, { line: pos.line, ch: range.to.ch + 1 }, 'url')
      ...
      if (
        ...
        const t2 = cm.hmdReadLink(mat[1], pos.line)
        if (!t2) url = null
        else {
          // remove title part (if exists)
