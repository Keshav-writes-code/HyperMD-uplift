...
export class HideToken implements Addon.Addon, Options {
  ...
  updateImmediately() {
    ...
    const activedLines: Record<number, OrderedRange[]> = {}
    const lastActivedLines = this._rangesInLine

    // update this._activedLines and caretAtLines
    for (const selection of selections) {
      const oRange = orderedRange(selection)
      const line0 = oRange[0].line,
        line1 = oRange[1].line
    ...
    cm.operation(() => {
      ...
      for (const line in activedLines) {
        const lineChanged = this.procLine(~~line)
