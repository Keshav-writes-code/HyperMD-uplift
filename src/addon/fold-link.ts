...
export const LinkFolder: FolderFunc = function (stream, token) {
  ...
  const spanExtractor = getLineSpanExtractor(cm)
  let tmpSpans: Span[]

  // first, find the link text span

  const linkTextSpan = spanExtractor.findSpanWithTypeAt(
  ...
  const linkHrefSpan = spanExtractor.findSpanWithTypeAt(
  ...
  const text = cm.getRange(href_from, href_to)
  ...
  const marker = cm.markText(href_from, href_to, {
    collapsed: true,
    replacedWith: img,
  })
