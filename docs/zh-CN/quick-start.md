...
2. 如果有不需要的扩展，可以在 _编辑器选项_ 里禁用。参考 [插件相关的编辑器选项][]
...
首先，_hypermd_ 在 JavaScript 里用 require 引入 CSS 文件，
...
> :warning: **还是闭包的问题**...
>
> CodeMirror 和 HyperMD **必须** 用相同的方法载入：要么用打包器, 要么 RequireJS 或者 `<script>` 标签。
> 如果不一致，HyperMD 可能会无法正常工作，因为它无法访问正确的 CodeMirror！
>
> 有的组件 (例如 SimpleMDE, React-CodeMirror) 使用了其 **私有的** CodeMirror 版本,
